...
if ( file_exists( CHCH_PFC_PLUGIN_DIR . 'admin/includes/CMB2/init.php' ) ) {
  require_once CHCH_PFC_PLUGIN_DIR . 'admin/includes/CMB2/init.php';
}
...
class ChChPopUpCloseAdmin {
  ...
  function __construct() {
    ...
    add_action( 'cmb2_init', array( $this, 'chch_pfc_initialize_cmb_meta_boxes' ) );
   ...
   * 
   */
  function chch_pfc_render_pages_select( $field, $escaped_value, $object_id, $object_type, $field_type_object ) {
   ...
   * 
   */
  function chch_pfc_render_cookie_select( $field, $escaped_value, $object_id, $object_type, $field_type_object ) {
  ...
  function chch_pfc_save_pop_up_meta( $post_id, $post, $update ) {
    ...
    if ( !isset( $_POST['chch_pfc_save_nonce'] ) || !wp_verify_nonce( $_POST['chch_pfc_save_nonce'], 'chch_pfc_save_nonce_' . $post_id ) ) {
      return;
    }
    ...
    if ( !empty( $template ) && !empty( $template_base ) ) {
      ...
      $template_data['size'] = array( 'size' => sanitize_text_field( $_REQUEST['_' . $template . '_size_size'] ), );
