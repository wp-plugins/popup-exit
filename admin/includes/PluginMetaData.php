 ...
 * description: extracts .php plugin meta info, config data, dependencies
 * priority: never
 * category: library
 * author: milky
 ...
 * 
 * 
 *  Utility code for reading plugin meta data (pmd) and dependencies
 *  and configuration info.
 *
 *  Upon reading all .php plugin files with ->scan(), it builds up two
 *  lists:
 *
 *     $this->plugin
 *         Contains all .php file description fields. As index into
 *         this array, the plugin id / basename is used. Each row
 *         contains fields like "title" and "type".
 *
 *     $this->config
 *         Holds configsetting values. The array index is the config
 *         varname, like "app[option]", and each row contains "type"
 *         "description" and default "value".
 *
 *  Additional id->id lists for dependencies are then extracted, too.
 *  And the plugin list can be filtered using ->by().
 *
 */
...
#      $this->pmd = & $this->plugin;   // reference (old name)
    ...
    * - returns a single row, but usually the data ends up in $this->plugin[] sorted by $id
    *
   ...
   function parse($src) {
      ...
      */
      ...
      #-- valid PMD only if it has one of the basic fields
   ...
   function scan($basedir) {
      ...
      foreach ($this->scan_subdirs($basedir) as $num=>$fn) {
         ...
         #-- basename == id
         $id = $this->basename($fn); // cut off .php and .inc and .cls
   ...
   function extract_lists() {
      ...
      foreach ($this->plugin as $id=>$e) {
         ...
         #-- provides:
         if (isset($e["provides"])) foreach (explode(",",$e["provides"]) as $set) if ($set=trim($set)) {
